...
trait SourceDocumentLike[-A] {
  ...
  /** The source parameter to provide to `wkhtmltopdf` Defaults to read from STDIN.
    */
  def commandParameter(sourceDocument: A): String = "-"

  /** Source the input of the process from this sourceDocument Defaults to passthrough.
    */
